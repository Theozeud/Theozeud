
<h1 align="center">Hi there 👋, I'm Théo Duez</h1>

<img align="right" src="https://github-readme-stats.vercel.app/api?username=Theozeud&show_icons=true&theme=radical" alt="GitHub Stats"  />

<h4 align="left"> 🎓 PhD student in applied mathematics, with a background in mathematics, and scientific computing.</h4>


<h4 align="left"> 🔬 Research Interests : Numerical analysis, PDEs, Quantum physics, Time-dependant density functional theory (TDDFT)   </h4>

<h4 align="left"> 💻 Programming Languages : </h4>
<p align="left">
        <a href="https://julialang.org/" target="_blank" rel="noreferrer"> <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/julia/julia-original-wordmark.svg" alt="Julia" width="40" height="40"/> </a>
        <a href="https://isocpp.org/" target="_blank" rel="noreferrer"> <img src="Assets/C++.svg" alt="C++" width="40" height="40"/> </a>
        <a href="https://www.python.org" target="_blank" rel="noreferrer"> <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/python/python-original.svg" alt="Python" width="40" height="40"/> </a>
        <a href="https://www.python.org" target="_blank" rel="noreferrer"> <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/matlab/matlab-original.svg" alt="Python" width="43" height="38"/> </a>
    </p>


## Selected projects

- **CEA Saclay — Nov 2022 → Mar 2023** : Lattice Boltzmann simulations for two-phase flows with d evelopment within the *LBM_saclay* C++ code.

- **Max Planck Institute — Apr 2023 → Aug 2023** : 
- Physics-informed neural networks (PINNs), in particular Hamiltonian & Lagrangian neural networks (HNN, LNN) and Symplectic and recurrent architectures (SympNet). In contributed mostly to the Julia package *GeometricMachineLearning.jl*.
